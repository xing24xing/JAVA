...
//      private JTextField cvvField; // JTextField for CVV input
 ...
 PreparedStatement ps = null;
    ...
    private void initComponents() {
        ...
        jLabel9.setText("Enter Debit Card No. ( 16 digit)");
        jPanel1.add(jLabel9, new org.netbeans.lib.awtextra.AbsoluteConstraints(600, 190, 260, -1));
    ...
    String debitCardNo = debitno.getText(); // Get debit card number input

    // Validate debit card number length
    if (debitCardNo.length() != 16) {
        JOptionPane.showMessageDialog(this, "Please enter a valid 16-digit debit card number.");
        return;
    }
    ...
    try {
        String query = "INSERT INTO CardDetails (CustomerID, Name, Email, Address, City, State, Pincode, Card_no, Exp_date, Cvv) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ps = con.prepareStatement(query);
        ps.setInt(1, userId);
        ...
        ps.setString(8, debitCardNo); // Set debit card number
        ps.setDate(9, expiryDate); // Set Date
        ps.setString(10, cardCvv);
